/*
	보고서에는 없는 코드.
	이 부분은 현재 Xenomai가 설치되어 있지 않아, 실행해보지 않은 코드이며
	현재 2023년 11월 28일(화)에 뮤텍스를 각 공유자원당 하나씩 만들어 제공하면 된다는
	가설을 갖고 작성한 코드입니다.
	현재 레포트에는 들어있지 않은 코드이며 검증이 되지 않은 코드입니다.
	-> 실제로 구동해본 결과, 뮤텍스 생성은 잘되나, 원하는대로 구현이 되지 않는다..
